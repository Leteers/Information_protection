...
def read_bmp_pixels_rgb(file_path):
    ...
    binary_pixel_data = [[format(pixel[0], '08b'), format(pixel[1], '08b'), format(pixel[2], '08b')] for pixel in pixel_data]
...
def LSB_Replacement(file_width, file_height, sha, pixel_data):
    ...
    for i in range(len(key)):
        ...
        test.append(pixel_data[key[i][0]])
    create_image_from_pixels(pixel_data, file_width,
                             file_height, output_file_path)
    new_file= read_bmp_pixels_rgb(output_file_path)
    delta = set(map(tuple, new_file))
    print(test==np.array([x for x in pixel_data if tuple(x) not in delta]))
